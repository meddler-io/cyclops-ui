...
@Component({
  selector: 'add-on-tools-runner',
  templateUrl: './add-on-tools-runner.component.html',
  styleUrls: ['./add-on-tools-runner.component.scss']
})
export class AddOnToolsRunnerComponent implements OnInit {
  ...
  external_tools_devsec_list = this.apiService.getExternalDevSecTools().pipe(
    ...
    map(_ => {
      return _.map(element => {
        let tool_name = Object.keys(element)
          .filter(f => f !== 'PARSE').filter(f => f !== 'alias')[0];

        return {
          name: tool_name
          ,
          alias: tool_name.toLowerCase()
          ,
          parse: element['PARSE']
          // parse: false
        };
  ...
  runExternalToolNew(tool, appUrl, gitUrl) {
    ...
    this.subscription_1 = this.selectedApp.pipe(
      ...
      // return;
      this.router.navigate(['external_tool_scans_new', _], {
        relativeTo: this.activatedRoute.parent
      })
